...
# Download latest model and scaler from Cloud Storage
def download_latest_model_and_scaler():
    print("=== [☁️] Downloading latest model and scaler from Cloud Storage ===")
    
    # 1. Try incremental learning model first
    incremental_model = get_latest_model_filename(GCS_BUCKET_NAME, prefix="models/incremental_")
    if incremental_model:
        ...
        scaler_blob = incremental_model.replace("incremental_", "scaler_incremental_").replace(".pt", ".pkl")
        download_from_gcs(GCS_BUCKET_NAME, scaler_blob, "/tmp/latest_scaler.pkl")
        return
    
    # 2. Try complete retraining model
    model_blob = get_latest_model_filename(GCS_BUCKET_NAME, prefix="models/final_model_")
    if model_blob:
        print("🔄 Using complete retraining model")
        download_from_gcs(GCS_BUCKET_NAME, model_blob, "/tmp/latest_model.pt")
        scaler_blob = model_blob.replace("final_model_", "scaler_standard_").replace(".pt", ".pkl")
        download_from_gcs(GCS_BUCKET_NAME, scaler_blob, "/tmp/latest_scaler.pkl")
    else:
        print("[⚠️] Latest model file not found.")

def main():
    print("=== [🚀] Timeseries Pipeline Starting ===")

    # Download latest model/scaler from Cloud Storage
    download_latest_model_and_scaler()

    # Load technical indicator data from database
